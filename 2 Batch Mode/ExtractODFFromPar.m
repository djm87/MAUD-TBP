function [phase] = ExtractODFFromPar(parName)
...
%
    ...
    for i=1:phase.num
        ...
        if phase.hasODF(i)==true
            ...
            if strcmp(text{2},'none')
                phase.ss{i}=specimenSymmetry('-1');
            elseif strcmp(text{2},'orthorhombic')
                phase.ss{i}=specimenSymmetry('222');
            else
    ...
    %                 odf(i,j,end)=NaN;
            ...
            %             S=sprintf('%6.2f %6.2f %6.2f %6.2f\n',...
            %                 (k-1)*res,(j-1)*res,(i-1)*res,odf(i,j,k));
            %             fprintf(fileID,'%s',S);
            %             fprintf('%8.6f ',odf(i,j,k))
                    end
            %         fprintf('\n');
                end
