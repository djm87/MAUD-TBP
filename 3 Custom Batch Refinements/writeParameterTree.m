...
function [par,count,c]=writeParameterTree(par,c,count,object,last,lvl,flag)
...
%                 if contains(c(count),'refln_index_h')
                ...
                while cnt<3000 %Assumes 3000 as max 
                    ...
                    elseif strcmp(flag,'write')
                        if ~all(strcmp(splitline,strcat(par.(fname).(var){cnt+1})))
                            c(count)=join(par.(fname).(var){cnt+1});
                        end
                    end
                    cnt=cnt+1;
                end
            ...
            elseif contains(c(count),object)
                lvl=lvl+1;
                [par.(fname),count,c]=WriteParameterTree(par.(fname),c,count,object,id(2),lvl,flag);
            end
        end
    end
  else
